...
- **Search command**: `grep -rn "\.id\b\|\.name\b\|\.email\b" <file>` to find property accesses that may need null guards
...
- **Search command**: `grep -rn "parseInt\|parseFloat\|Number(\|String(\|toString" <file>` to find type conversion points
...
- **Search command**: `grep -rn "\.length\|\.size\|\.count\|\.slice\|\.substring" <file>` to find boundary operations
...
- **Search command**: `grep -rn "\.then(\|new Promise\|async " <file>` to find async patterns
...
- **Search command**: `grep -rn "\.push(\|\.splice(\|\.sort(\|= \[" <file>` to find mutation operations
...
- **Search command**: `grep -rn "import\|require(\|from " <file>` to trace imports
...
- **Search command**: `grep -rn "Promise\.all\|Promise\.race\|setTimeout\|setInterval" <file>` to find concurrency patterns
...
- **Search command**: `grep -rn "process\.env\|os\.environ\|env::\|getenv" <file>` to find env var usage
...
2. **Similar pattern search**: `grep -rn "<error_pattern>" src/` to find if the same mistake exists elsewhere
3. **Call chain tracing**: `grep -rn "import.*from.*<module>" src/` to find all callers of the failing module
4. **Configuration search**: `grep -rn "<config_key>" .` to find where a config value is set vs. read
5. **Dead code detection**: `grep -rn "function <name>\|export.*<name>" src/` then `grep -rn "<name>" src/ | wc -l` — if only 1 match, it may be unused
